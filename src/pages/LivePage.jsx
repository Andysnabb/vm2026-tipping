    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                // Store extracted actuals (groups + knockout) in localStorage so LeaderboardPage can pick them up
                try {
                    const liveActual = {
                        groups: extractStandingsGroups(standingsData).reduce((acc, g) => {
                            // g.label may be like 'Group A' or 'Group A - Xxx', try to extract letter
                            const match = String(g.label || "").match(/([A-L])/i);
                            const key = match ? match[1].toUpperCase() : null;
                            if (key) acc[key] = g.rows.map(r => r.team);
                            return acc;
                        }, {}),
                        // simple knockout mapping: attempt to capture winners from matches array
                        knockout: {
                            // We'll leave these empty; manual admin can fill knockout later if necessary
                        }
                    };
                    try {
